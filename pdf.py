...
# ═══════════════════════════════════════════════════════════════
#  HELPERS (Updated to guarantee titles stick to content)
# ═══════════════════════════════════════════════════════════════

def flush_orphan(flowable):
    """Takes any waiting section titles and merges them into an unbreakable block with the new content."""
    global _orphan_buffer
    if _orphan_buffer:
        _orphan_buffer.append(flowable)
        story.append(KeepTogether(_orphan_buffer))
        _orphan_buffer = []
    else:
...
def section(text):
    global _orphan_buffer
    _orphan_buffer.append(Paragraph(text, SEC))

def sub(text):
    global _orphan_buffer
    _orphan_buffer.append(Paragraph(text, SSEC))

def body(text):
    flush_orphan(Paragraph(text, BODY))

def bullet(text):
    flush_orphan(Paragraph(f'• {text}', BULL))

def key(label, text):
    flush_orphan(Paragraph(f'<b>{label}:</b> {text}', BODY))

def formula(text):
    flush_orphan(Paragraph(text, FORM))

def note(text):
    flush_orphan(Paragraph(f'★ {text}', NOTE))

def rule():
    flush_orphan(HRFlowable(width='100%', thickness=0.5, color=C_RULE, spaceAfter=4, spaceBefore=2))
