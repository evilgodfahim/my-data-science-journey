...
def section(text):
    p = Paragraph(text, SEC)
    p.keepWithNext = True  # Ensures the heading sticks to the text below it
    story.append(p)

def sub(text):
    p = Paragraph(text, SSEC)
    p.keepWithNext = True  # Ensures the sub-heading sticks to the text below it
    story.append(p)
...
def box_table(rows, col_widths=None, header_row=True):
    ...
    # KeepTogether ensures the table never splits across two pages
